...
const MECHANICS: Record<GameKey, Record<string, unknown>[]> = {
  vice: [
    ...
    { id: 'tower-multipliers', kind: 'win-modifier', enabled: true, affectsMath: true,
      mathBinding: ['custom.towerMultiplierWeights', 'custom.towerMultiplierRule', 'custom.towerMultiplierStickyRule', 'custom.towerMultiplierOnHotSpins'],
      params: {
        weights: 'towerMultiplierWeights [55,20,9,6,10] = x1..x5; every reel standing FULLY WILD in a free spin is dealt one',
        apply: 'HIGHEST crossed badge — NOT the product (product measures 187% RTP on a bought 4-scatter round) and NOT the sum (floors at 90.53%)',
        never: 'scatter pay is never multiplied; nothing stacks on the 5-wild instant max win; hot spins carry no badge',
        sticky: 'a 4sc sticky tower keeps the badge it was dealt when it JOINED for the whole round; a 3sc round re-expands each spin and draws fresh badges',
        randomness: 'CRITICAL: draw the badge from a RESERVED seed namespace (ours: keccak(seed, 1<<200)) so the reel-stop stream is untouched — reels must land identically with the mechanic on or off, or every certified RTP here is void for your build',
        presentation: 'art badge x1..x5 (theme/vice/wild_multi_sheet.webp, 5 frames) seated in the lower third of the column (slotYFrac 0.86, sizeFrac 0.82) so it clears the WILD lettering; it POPS on when it locks — see visualParams expandWildMultiPop / expandWildMultiPopTime',
      }, compatibleGrids: ['5x5'] },
    ...
    { id: 'buy-stages', kind: 'base-feature', enabled: true, affectsMath: true,
      ...
      params: {
        presentation: 'forced stops carry the bought scatter count — 2 land, tease arms, rest drop like a natural trigger; board evaluated in full (display == payout)',
        guaranteedTower: '4-scatter buy only: guaranteedTowerOnFirstSpin — if the first free spin would land with NO fully-wild reel, guaranteedTowerReel is advanced to the next stop whose window holds a wild. Without it 15.5% of bought rounds (1 in 6.4) showed no tower at all on a buy sold as "10 sticky tower spins"; now 0%, fires on 83.6% of rounds, mean 2.03 towers at round end. Bought rounds ONLY — never a natural or ante trigger. Priced in: the buy stays 200x.',
        stripSwapWarning: 'when a stage swaps in its own strips, swap reelLengths WITH them — we shipped a round where they desynced and the display rolled a different board than the settlement scored, so wins highlighted the wrong cells',
      } },

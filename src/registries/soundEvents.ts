import { RegistryEntry, createRegistry } from './types.js';

// All sound events are now wired through src/audio/SoundManager.ts (Howler.js)
// and fired by:
//   - src/audio/useSoundLayer.ts        — state-driven sounds (spin, win tiers,
//                                          free-spin trigger, ambient music)
//   - src/game/ReelSet.ts audioHooks    — per-reel-stop and scatter-land
//
// Default audio files ship in public/audio/<id>.ogg sourced from Kenney.nl
// (CC0). See docs/SOUND_PACK_SOURCES.md for full attribution. Generated games
// inherit the same wiring; the assembler emits the import + hook setup
// (Checkpoint 4d).

export interface SoundEventEntry extends RegistryEntry {
  event: string;
  trigger: string;
  loop: boolean;
  priority: 'high' | 'medium' | 'low';
}

const entries: readonly SoundEventEntry[] = [
  {
    id: 'spin-start',
    name: 'Spin Start',
    description: 'Short whoosh sound when reels begin spinning.',
    version: '0.1.0',
    implemented: true,
    event: 'spin:start',
    trigger: 'spin button click',
    loop: false,
    priority: 'high',
  },
  {
    id: 'reel-stop',
    name: 'Reel Stop',
    description: 'Thud/click when each reel lands. Played 5 times per spin with stagger.',
    version: '0.1.0',
    implemented: true,
    event: 'reel:stop',
    trigger: 'each reel landing',
    loop: false,
    priority: 'high',
  },
  {
    id: 'win-small',
    name: 'Small Win',
    description: 'Brief chime for wins under 2× bet.',
    version: '0.1.0',
    implemented: true,
    event: 'win:small',
    trigger: 'win < 2× bet',
    loop: false,
    priority: 'medium',
  },
  {
    id: 'win-big',
    name: 'Big Win',
    description: 'Escalating fanfare for wins 10×–50× bet.',
    version: '0.1.0',
    implemented: true,
    event: 'win:big',
    trigger: 'win 10×–50× bet',
    loop: false,
    priority: 'high',
  },
  {
    id: 'win-mega',
    name: 'Mega Win',
    description: 'Epic celebration for wins 50×+ bet.',
    version: '0.1.0',
    implemented: true,
    event: 'win:mega',
    trigger: 'win >= 50× bet',
    loop: false,
    priority: 'high',
  },
  {
    id: 'win-marquee',
    name: 'Win Marquee Music',
    description: 'Celebration track under the BIG/MEGA/EPIC/MAX marquee — starts with the marquee, fades out with it (fast on skip).',
    version: '0.1.0',
    implemented: true,
    event: 'win:marquee',
    trigger: 'marquee celebration start (win >= minBigWin x bet)',
    loop: false,
    priority: 'high',
  },
  {
    id: 'wild-land',
    name: 'Wild Land',
    description: 'Cash-bundle drop (thud + bill flaps) when a money wild lands on the board.',
    version: '0.1.0',
    implemented: true,
    event: 'wild:land',
    trigger: 'wild visible on reel stop',
    loop: false,
    priority: 'high',
  },
  {
    id: 'wild-expand',
    name: 'Wild Expand',
    description: 'Accelerating bill-riffle riser into a fat slam as the wild tower races up and locks in.',
    version: '0.1.0',
    implemented: true,
    event: 'wild:expand',
    trigger: 'expanding-wild reveal starts',
    loop: false,
    priority: 'high',
  },
  {
    id: 'scatter-land',
    name: 'Scatter Land',
    description: 'Distinctive sound when a scatter symbol becomes visible.',
    version: '0.1.0',
    implemented: true,
    event: 'scatter:land',
    trigger: 'scatter visible on reel stop',
    loop: false,
    priority: 'high',
  },
  {
    id: 'free-spin-trigger',
    name: 'Free Spin Trigger',
    description: 'Triumphant sound when 3+ scatters trigger the bonus.',
    version: '0.1.0',
    implemented: true,
    event: 'freespin:trigger',
    trigger: 'scatterCount >= 3',
    loop: false,
    priority: 'high',
  },
  {
    id: 'near-miss-tease',
    name: 'Near-Miss Tease',
    description: 'Rising tension tone when a reel enters its near-miss slowdown phase.',
    version: '0.1.0',
    implemented: true,
    event: 'nearmiss:tease',
    trigger: 'reel entering near-miss deceleration',
    loop: false,
    priority: 'high',
  },
  {
    id: 'win-normal',
    name: 'Normal Win',
    description: 'Mid-tier chime for wins 2×–10× bet. Bridges small and big win stings.',
    version: '0.1.0',
    implemented: true,
    event: 'win:normal',
    trigger: 'win 2×-10× bet',
    loop: false,
    priority: 'medium',
  },
  {
    id: 'reel-spin-loop',
    name: 'Reel Spin Loop',
    description: 'Looping mechanical whir while reels are in motion. Stops when all reels land.',
    version: '0.1.0',
    implemented: true,
    event: 'reel:spinning',
    trigger: 'reels spinning',
    loop: true,
    priority: 'low',
  },
  {
    id: 'coin-chime',
    name: 'Coin Chime',
    description: 'Short coin impact accent used during big+ win celebrations.',
    version: '0.1.0',
    implemented: true,
    event: 'celebration:coin',
    trigger: 'big/mega win celebration timeline',
    loop: false,
    priority: 'medium',
  },
  {
    id: 'win-tally-tick',
    name: 'Win Tally Tick',
    description: 'Coin tick fired on a steady grid during the marquee count-up; pitch rises with progress (Pragmatic-style tally).',
    version: '0.1.0',
    implemented: true,
    event: 'celebration:tally-tick',
    trigger: 'marquee count-up progress grid (~70ms)',
    loop: false,
    priority: 'medium',
  },
  {
    id: 'win-tally-end',
    name: 'Win Tally Terminator',
    description: 'The terminator hit when the count-up lands on the final amount: thump + coin splash + bright ring.',
    version: '0.1.0',
    implemented: true,
    event: 'celebration:tally-end',
    trigger: 'marquee count-up completes',
    loop: false,
    priority: 'high',
  },
  {
    id: 'multi-fly',
    name: 'Multi Fly',
    description: 'Whoosh as a gift multiplier value flies off its crate (to the pool in free spins, to the plaque in the base game).',
    version: '0.1.0',
    implemented: true,
    event: 'multi:fly',
    trigger: 'gift multiplier flight starts',
    loop: false,
    priority: 'high',
  },
  {
    id: 'multi-collect',
    name: 'Multi Collect',
    description: 'Impact when a flying multiplier value docks into the pool badge / win plaque (star-dust arrival).',
    version: '0.1.0',
    implemented: true,
    event: 'multi:collect',
    trigger: 'multiplier value arrives at pool/plaque',
    loop: false,
    priority: 'high',
  },
  {
    id: 'multi-apply',
    name: 'Multi Apply (Win xN)',
    description: 'The multiplication moment: the plaque squeezes "win xN" together and the product pops in.',
    version: '0.1.0',
    implemented: true,
    event: 'multi:apply',
    trigger: 'win x multiplier resolves to the product',
    loop: false,
    priority: 'high',
  },
  {
    id: 'fs-counter-down',
    name: 'FS Counter Down',
    description: 'Wheel tick as the free-spins counter rolls one down at spin start.',
    version: '0.1.0',
    implemented: true,
    event: 'fscounter:down',
    trigger: 'FS counter rolls down',
    loop: false,
    priority: 'medium',
  },
  {
    id: 'fs-counter-up',
    name: 'FS Counter Up',
    description: 'Rising tick as the free-spins counter rolls UP on a retrigger (+5).',
    version: '0.1.0',
    implemented: true,
    event: 'fscounter:up',
    trigger: 'FS counter rolls up (retrigger)',
    loop: false,
    priority: 'medium',
  },
  {
    id: 'fs-retrigger',
    name: 'FS Retrigger',
    description: 'Celebratory sting when 3+ scatters land during free spins and add +5 spins.',
    version: '0.1.0',
    implemented: true,
    event: 'fs:retrigger',
    trigger: 'retrigger (3+ scatters in a FS spin)',
    loop: false,
    priority: 'high',
  },
  {
    id: 'tier-up',
    name: 'Tier Promotion',
    description: 'Impact stinger when the marquee promotes a tier mid-count (BIG→MEGA→EPIC→MAX); pitched up per tier.',
    version: '0.1.0',
    implemented: true,
    event: 'celebration:tier-up',
    trigger: 'marquee tier promotion',
    loop: false,
    priority: 'high',
  },
  {
    id: 'tease-riser',
    name: 'Anticipation Riser',
    description: 'Noise-sweep + in-key gliss + accelerating pulse while the scatter tease runs; music ducks under it. Cut on resolve.',
    version: '0.1.0',
    implemented: true,
    event: 'nearmiss:riser',
    trigger: '2nd visible scatter (tease engages)',
    loop: false,
    priority: 'high',
  },
  {
    id: 'tease-miss',
    name: 'Tease Miss Thud',
    description: 'Damped dead-stop thud when the tease resolves WITHOUT the trigger — followed by a near-silence beat before the music returns.',
    version: '0.1.0',
    implemented: true,
    event: 'nearmiss:miss',
    trigger: 'tease resolves without 3+ scatters',
    loop: false,
    priority: 'high',
  },
  {
    id: 'ambient-music',
    name: 'Background Music',
    description: 'Looping ambient music during gameplay. Separate tracks for base and bonus.',
    version: '0.1.0',
    implemented: true,
    event: 'music:ambient',
    trigger: 'game load',
    loop: true,
    priority: 'low',
  },
  // PER-SYMBOL win voices ("quips"): when a winning line is revealed, the
  // symbol that won speaks — the goat bleats, the cow moos, the corn pops.
  // Far more characterful than one generic win chime, and it tells the player
  // WHAT paid. Ids are keyed by SymbolId (0..8); a missing file just no-ops.
  ...([0, 1, 2, 3, 4, 5, 6, 7, 8] as const).map(n => ({
    id: `quip-${n}`,
    name: `Symbol Voice ${n}`,
    description: `Per-symbol win voice for SymbolId ${n} (fires as its line is revealed).`,
    version: '0.1.0',
    implemented: true,
    event: `win:quip:${n}`,
    trigger: 'winning line revealed for this symbol',
    loop: false,
    priority: 'medium' as const,
  })),
] as const;

export const soundEventRegistry = createRegistry(entries, { compatibleGrids: ['5x3', '5x5'] });

...
if ROWS == 5:
    ...
    STRIPS = [
        build_strip({W: 1, SC: 1, 2: 4, 3: 2, 4: 8, 5: 2, 6: 10, 7: 2, 8: 10}),  # R1: NOW carries a wild (Noski: full-board 5-tower max-win)
...
SIMUL_MULTS = {}  # 1-4 wilds pay natural ways; 5 = instant max (handled in eval_spin)
...
# ── FS strips: base symbol proportions scaled to FS_LEN, ONE rare wild ea. ──
FS_LEN = int(os.environ.get('VH_FS_STRIP_LEN', '120'))
def build_strip_n(counts, n):
    st=[]
    for sym,c in counts.items(): st += [sym]*c
    assert len(st)==n,(len(st),n)
    random.Random(4242+n+len(st)).shuffle(st); return st
def fs_strip(base):
    fillers={k:v for k,v in base.items()}
    tot=sum(fillers.values())
    sc={k:max(1,round(v/tot*(FS_LEN-2))) for k,v in fillers.items()}
    diff=(FS_LEN-2)-sum(sc.values()); ks=sorted(sc,key=lambda k:-sc[k]); i=0
    while diff!=0:
        sc[ks[i%len(ks)]] += 1 if diff>0 else -1; diff += -1 if diff>0 else 1; i+=1
    return build_strip_n({W:1, SC:1, **sc}, FS_LEN)
if ROWS==5:
    BASE_FS=[{2:4,3:2,4:8,5:2,6:10,7:2,8:10},{2:4,3:7,4:2,5:8,6:2,7:13,8:2},{2:4,3:2,4:8,5:2,6:10,7:2,8:10},{2:4,3:7,4:2,5:8,6:2,7:13,8:2},{2:4,3:2,4:8,5:2,6:10,7:2,8:10}]
    FS_STRIPS=[fs_strip(b) for b in BASE_FS]
else:
    FS_STRIPS=STRIPS
FS_PRE=precompute(FS_STRIPS)
FS_LENS=[len(s) for s in FS_STRIPS]



def eval_spin(stops, per_spin_expand, sticky_set, pays, scatter_pay, pre=None):
    ...
    pre = pre if pre is not None else PRE
    ...
    if per_spin_expand:
        for i in range(REELS):
            if data[i][1] > 0:
                full.add(i)
    # FULL BOARD (5 wild reels) = INSTANT MAX WIN (Noski): opens + connects -> 5000x marquee.
    if len(full) >= REELS:
        return MAX_WIN_X * 10000, 0, full
    ...
    for _ in range(n):
        ...
        if sc >= 3:
            ...
            while fs_left > 0 and fs_played < cap:
                ...
                st = [rng.randrange(FS_LENS[r]) for r in range(REELS)]
                if sticky_round:
                    ...
                    for i in range(REELS):
                        ...
                        if i not in sticky and FS_PRE[i][st[i]][1] > 0:
                            sticky.add(i)
                    w2, sc2, _ = eval_spin(st, False, sticky, pays, scatter_pay, FS_PRE)
                    # FULL HOUSE: all towers standing -> the spin pays xN.
                    if len(sticky) >= STICKY_CAP:
                        w2 *= STICKY_FULL_MULT
                else:
                    w2, sc2, full = eval_spin(st, True, sticky, pays, scatter_pay, FS_PRE)

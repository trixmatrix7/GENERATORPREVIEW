...
// GSAP, driven by the app ticker). Fired per winning combination.
//
// THIS IS THE EXACT RUNTIME FILE — drop it in 1:1. It only imports pixi.js + gsap.
...
export interface WaysLightConfig {
  ...
  stepMs: number;
  /** Cell size, set by your reel layer so the comet head scales with the grid. */
  cellSize: number;
}

/** Live, mutable config — updated by applyVisualParam, read on each play. */
...
/** Kill every in-flight comet immediately (call from your clearHighlights). */
 ...
 * (reel-local coords) grouped left→right by reel. Self-cleaning.
 */

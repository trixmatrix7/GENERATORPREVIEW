...
export class PixiApp {
  ...
  /** Per-symbol LANDING spritesheet: plays ONCE as the symbol settles, then
   *  hands back to the static art. Same slicing rules as setSymbolWinSheet. */
  async setSymbolLandSheet(symbolId: number, url: string | null, cols: number, rows: number, count: number, fps = 12): Promise<void> {
    const old = SYMBOL_LAND_SHEETS.get(symbolId);
    SYMBOL_LAND_SHEETS.delete(symbolId);
    if (old) for (const f of old.frames) { try { f.destroy(true); } catch { /* torn down */ } }
    if (!url) return;
    try {
      const frames = await this.sliceSheetHD(url, cols, rows, count);
      SYMBOL_LAND_SHEETS.set(symbolId, { frames, fps });
    } catch (err) {
      console.warn('[PixiApp] failed to load symbol land sheet:', err);
    }
  }
  ...
  private async resolveTumble(outcome: FruitStacksOutcome, tokenSymbol: string, decimals: number): Promise<void> {
    ...
    if (round.fsTriggered && round.fsSpins.length > 0 && !prefersReducedMotion()) {
      ...
      if (round.buyStage === 0) {
        ...
        walkSc(this.reelSet.container);
        for (const c of scatterCells) c.play('win');
        // Bend spielt EINMAL durch (53f @30fps ≈ 1.77s) → dann FS-Transition.
        await new Promise<void>(r => { gsap.delayedCall(this.turbo ? 0.6 : 1.85, () => r()); });
        if (!this.isLive) return;
      }
  ...
  private async playTumbleSpin(spin: FruitSpin, decimals: number, fsCtx?: { fs: boolean; roundWinBefore: bigint }): Promise<void> {
    ...
    if (fsCtx?.fs && spin.crates.length > 0 && !prefersReducedMotion()) {
      ...
      if (!this.isLive) return;
      // WINNA Pool-Apply am SPIN-ENDE (Noski: "man sieht den Multi nie drauf
      // fliegen"): wenn dieser Spin gewonnen hat, fliegt der ANGEWENDETE Pool
      // als Komet vom Badge auf die Plaque und die Zahl swappt aufs PRODUKT.

...
| [`hot-spins/`](./hot-spins/) | A 1-in-80 base spin runs **hot**: every reel holding a wild expands to a full wild reel. The base game's own feature — **the ante drops to ~84 % without it**. Never on a bought round; can suppress a trigger (certified); five hot reels = a base-game max win. |

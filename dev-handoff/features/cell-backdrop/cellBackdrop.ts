// cellBackdrop.ts — the PER-SYMBOL cell backdrop: a rounded pocket drawn behind
// EACH symbol tile so it travels with the symbol (spins during the spin, lands
// with it, sits permanently around the resting symbol). NOT a fixed grid.
//
// Module singleton so every symbol reads the same live config; your param
// handler updates it and redraws all symbol tiles. Drop this file in as-is
// (it imports the colour helpers from ./color).
 ...
 *  Call this FIRST in your per-symbol tile draw, before the symbol art. */
